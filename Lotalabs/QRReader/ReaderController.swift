...
extension QRCodeReaderViewController: AVCaptureMetadataOutputObjectsDelegate {
    ...
    func metadataOutput(_ captureOutput: AVCaptureMetadataOutput,
        ...
        if let metadataObject = metadataObjects.first {
            ...
            // ✅ qr코드가 가진 문자열이 URL 형태를 띈다면 출력.(아무런 qr코드나 찍는다고 출력시키면 안되니까 여기서 분기처리 가능. )// URL 형식은 없앴엉 내가
                //print(stringValue)
                //stringValue ==> 이 값이 기기 고유 아이디야
            if let FInfoVC = self.storyboard?.instantiateViewController(withIdentifier: "FInfoView") as? FriendInfoViewController {
                
                FInfoVC.uuid = stringValue
                    self.present(FInfoVC, animated: true)
                }
